...
def consultar_virustotal_hash(file_hash, api_key):
    """Consulta el hash de un archivo en VirusTotal v3."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "encontrado": False,
                "mensaje": (
                    "El archivo no se encuentra en la base de datos de"
                    " VirusTotal."
                ),
            }
            ...
            return {
                "error": f"Error en la API de VT (Status {response.status_code})"
            }
...
def extract_strings_and_iocs(file_bytes):
    ...
    domains = [
        d
        for d in domains
        if not d.endswith((".dll", ".sys", ".exe", ".manifest"))
    ][:10]

    return {"ips": ips[:10], "urls": urls[:10], "domains": domains}
...
def generate_dynamic_sandbox_report(file_name, file_size, entropy):
    ...
    mitre_attack = [
        ...
        },
        {
            ...
            "id": "T1027.002",
            "name": "Software Packing / High Entropy",
            "confidence": "Crítica",
        },
        {
            "tactic": "Persistence",
        ...
        },
        {
            ...
            "id": "T1071.001",
            "name": "Web Protocols (HTTP/HTTPS Exfiltration)",
            "confidence": "Media",
        },
        {
            "tactic": "Discovery",
    ...
    processes = [
        {
            "pid": 4128,
            "process": file_name,
            "path": f"C:\\Users\\Victim\\AppData\\Local\\Temp\\{file_name}",
    ...
    filesystem = [
        {
            "op": "CREAR_ARCHIVO",
            "path": (
                "C:\\Users\\Victim\\AppData\\Roaming\\Microsoft\\Vault\\keys.dat"
            ),
            "size": "14 KB",
        },
        {
        ...
        },
        {
            "op": "ELIMINAR",
            "path": (
                "C:\\Users\\Victim\\AppData\\Local\\Temp\\shadow_copy_backup.tmp"
            ),
            "size": "0 KB",
        },
    ]
    ...
    network_dns = [
        {
            "query": "c2-command-node.darknet-dns.org",
            "type": "A",
        ...
        },
        {
            ...
            "type": "A",
            "resolved_ip": "149.154.167.220",
            "status": "NOERROR",
        },
        {
            "query": "exfil-server-drop.xyz",
            "type": "A",
            "resolved_ip": "91.215.102.14",
            "status": "NXDOMAIN",
        },
    ]
    ...
    yara_matches = [
        {
            "rule": "SUSP_PE_High_Entropy_Packed",
            "namespace": "malware_packers",
            "tags": ["packed", "UPX", "Obfuscated"],
        },
        {
            "rule": "INDICATOR_TOOL_PS_EncodedCommand",
            "namespace": "execution_evasion",
            "tags": ["PowerShell", "Base64"],
        },
        {
            "rule": "MALWARE_Win_AgentTesla_Behavior",
            "namespace": "stealer",
            "tags": ["InfoStealer", "Keylogger"],
        },
    ]
    ...
    sigma_rules = [
        {
            ...
            "level": "High",
            "id": "proc_creation_win_powershell_base64",
        },
        {
            "title": "Persistence via Windows Run Registry Key",
            "level": "Medium",
    ...
    timeline = [
        {
            "time": "00:00:00",
            "event": (
                "Ejecución del binario primario en entorno GUI aislado."
            ),
            "severity": "Info",
        },
        {
            "time": "00:00:03",
            "event": (
                "Desempaquetado en memoria y desempaquetamiento de recursos"
                " PE."
            ),
            "severity": "Medio",
        },
        {
            "time": "00:00:07",
            "event": (
                "Creación de proceso subyacente powershell.exe con flags"
                " ocultas (-W Hidden)."
            ),
            "severity": "Alto",
        },
        {
            "time": "00:00:12",
            "event": (
                "Modificación de claves de registro para persistencia tras"
                " reinicio."
            ),
            "severity": "Crítico",
        },
        {
            "time": "00:00:18",
            "event": (
                "Intento de conexión saliente HTTP POST a C2 server"
                " (185.220.101.5)."
            ),
            "severity": "Crítico",
        },
...
def generate_pdf_report(
    filename, size_str, hashes, entropy, report, iocs, os_target
):
    """Genera un reporte PDF con estilo profesional SOC utilizando WeasyPrint."""
    mitre_rows = "".join([
        f"<tr><td><b>{m['tactic']}</b></td><td><code>{m['id']}</code></td><td>{m['name']}</td><td><span"
        f" style='color:#d29922;'>{m['confidence']}</span></td></tr>"
    ...
    ioc_ips = ", ".join(
        list(set(iocs["ips"] + ["185.220.101.5", "149.154.167.220"]))
    )
    ioc_doms = ", ".join(
        list(
            set(
                iocs["domains"]
                + ["c2-command-node.darknet-dns.org", "exfil-server-drop.xyz"]
            )
        )
    )
...
os_target = st.sidebar.selectbox(
    "Entorno Virtualizado (VM):",
    [
...
analysis_timeout = st.sidebar.slider(
    "Tiempo de Ejecución (segundos):", 30, 300, 60
)
enable_network = st.sidebar.checkbox(
    "Simular Red (INetSim / Tor Sinkhole)", value=True
)

st.sidebar.markdown("---")

# Intenta obtener la API Key de los Secrets (Cloud/Local); si no existe la clave o el archivo, atrapa la excepción
try:
    vt_api_key = st.secrets.get("VT_API_KEY", None)
except Exception:
    vt_api_key = None

# Si no hay API Key guardada en Secrets, muestra el campo para ingresarla manualmente
if not vt_api_key:
    st.sidebar.subheader("🔑 Integraciones")
    vt_api_key = st.sidebar.text_input(
        "VirusTotal API Key",
        type="password",
        help="Ingresá tu API Key de VirusTotal si no usás Secrets",
    )
...
st.caption(
    "Entorno seguro para la inspección profunda de amenazas, extracción de IOCs"
    " y mapeo MITRE ATT&CK."
)

# Carga de Archivos (Drag & Drop)
uploaded_file = st.file_uploader(
    "📥 Arrastra y suelta un archivo sospechoso para iniciar el análisis",
    type=None,
)
...
if uploaded_file is not None:
    ...
    report = generate_dynamic_sandbox_report(
        uploaded_file.name, size_str, entropy
    )
    ...
    with col_v1:
        ...
        if report["verdict"] == "CRÍTICO (Malware Confirmado)":
            st.markdown(
                f"**Veredicto:** <span"
                f" class='badge-critical'>{report['verdict']}</span>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f"**Veredicto:** <span"
                f" class='badge-high'>{report['verdict']}</span>",
                unsafe_allow_html=True,
            )
    ...
    with col_v2:
        st.metric(
            ...
            delta="Riesgo Elevado",
            delta_color="inverse",
        )

    with col_v3:
        st.metric(
            label="Entropía del Archivo",
            value=f"{entropy}",
    ...
    with tab_summary:
        ...
        with col_s1:
            ...
            st.error(
                "1. **Aislamiento Inmediato:** Bloquear la dirección IP"
                " `185.220.101.5` y el dominio `c2-command-node.darknet-dns.org` en"
                " el Firewall de borde/EDR."
            )
            st.warning(
                "2. **Remediación en Hosts:** Eliminar el valor de registro en"
                " `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\UpdaterAgent`"
                " en los endpoints afectados."
            )
            st.info(
                "3. **Cacería de Amenazas (Threat Hunting):** Ejecutar un"
                " barrido de red en la infraestructura buscando coincidencias con"
                " el SHA256."
            )
        ...
        with col_s2:
            ...
            fig_gauge.update_layout(
                paper_bgcolor="#0b0f19", font={"color": "white"}, height=280
            )
...
SHA256: {hashes['sha256']}
        ...
        with col_st1:
            ...
            df_props = pd.DataFrame([
                ...
                {
                    "Propiedad": "Arquitectura Evaluada",
                    "Valor": "x86 / x64 PE Executable",
                },
            ])
            st.table(df_props)
        ...
        # --- SECCIÓN INTELIGENCIA VIRUSTOTAL ---
        st.markdown("---")
        st.subheader("🦠 Inteligencia Global VirusTotal")

        if not vt_api_key:
            st.info(
                "💡 Ingresá tu **VirusTotal API Key** en la barra lateral para"
                " consultar los motores antivirus."
            )
        else:
            if st.button("🔍 Consultar Hash en VirusTotal"):
                with st.spinner(
                    "Consultando inteligencia de amenazas en VirusTotal..."
                ):
                    res = consultar_virustotal_hash(
                        hashes["sha256"], vt_api_key
                    )

                    if "error" in res:
                        st.error(res["error"])
                    elif not res["encontrado"]:
                        st.info(res["mensaje"])
                    else:
                        st.success("✅ Datos recuperados de VirusTotal")
                        col_vt1, col_vt2, col_vt3 = st.columns(3)
                        col_vt1.metric("Maliciosos 🚨", res["malicious"])
                        col_vt2.metric("Sospechosos ⚠️", res["suspicious"])
                        col_vt3.metric(
                            "Limpios / Indetectados ✅",
                            res["harmless"] + res["undetected"],
                        )

                        if res["malicious"] > 0:
                            st.error(
                                f"Detectado como malicioso por {res['malicious']} motores de seguridad."
                            )
                        else:
                            st.success(
                                "No se detectaron firmas maliciosas en VirusTotal."
                            )
    ...
    with tab_dynamic:
        ...
        with col_d1:
            ...
            st.dataframe(
                pd.DataFrame(report["registry"]), use_container_width=True
            )

        with col_d2:
            st.subheader("📁 Cambios en el Sistema de Archivos")
            st.dataframe(
                pd.DataFrame(report["filesystem"]), use_container_width=True
            )
    ...
    with tab_network:
        ...
        st.markdown("---")
        st.subheader(
            "🔍 Indicadores de Compromiso (IOCs) Extraídos del Binario"
        )
        ...
        with col_ioc1:
            ...
            all_ips = list(
                set(extracted_iocs["ips"] + ["185.220.101.5", "149.154.167.220"])
            )
            for ip in all_ips:
        ...
        with col_ioc2:
            ...
            all_domains = list(
                set(
                    extracted_iocs["domains"]
                    + [
                        "c2-command-node.darknet-dns.org",
                        "exfil-server-drop.xyz",
                    ]
                )
            )
        ...
        with col_ioc3:
            ...
            all_urls = list(
                set(extracted_iocs["urls"] + ["http://185.220.101.5/gate.php"])
            )
            for url in all_urls:
    ...
    with tab_rules:
        ...
        with col_r2:
            ...
            for sigma in report["sigma"]:
                with st.expander(
                    f"[{sigma['level'].upper()}] {sigma['title']}",
                    expanded=True,
                ):
    ...
    with tab_timeline:
        ...
        for item in report["timeline"]:
            sev_color = (
                "🔴"
                if item["severity"] in ["Crítico", "Alto"]
                else "🟡"
                if item["severity"] == "Medio"
                else "🔵"
            )
            st.markdown(
                f"**`[{item['time']}]`** {sev_color} **{item['event']}**"
    ...
    st.info(
        "👆 Por favor, carga un archivo ejecutable, documento o script arriba"
        " para iniciar la secuencia de análisis."
    )
    ...
    with col1:
        st.markdown(
            "**🔍 Análisis Estático de Archivos:** Cálculo automático de"
            " metadatos clave y firmas criptográficas (`MD5`, `SHA-1`, `SHA-256`)."
        )
        st.markdown(
            "**📊 Entropía Criptográfica:** Visualizaciones interactivas con Plotly"
            " para detectar ofuscación, empaquetado o cifrado."
        )
        st.markdown(
            "**⚠️ Extracción de IOCs & Veredicto:** Clasificación dinámica de"
            " amenaza e identificación de artefactos maliciosos."
        )

    with col2:
        st.markdown(
            "**📄 Reportes PDF:** Exportación de informes técnicos listos para"
            " auditoría mediante WeasyPrint."
        )
        st.markdown(
            "**🖥️ Interfaz SOC CyberDark:** Dashboard optimizado por pestañas con"
            " estética oscura de operaciones."
        )